...
- (UIImage *)imageByRenderingViewByRect:(CGRect)rect {
    ...
    CGRect imgRect = rect;
    imgRect.origin.x *= resultingImage.scale;
    imgRect.origin.y *= resultingImage.scale;
    imgRect.size.width *= resultingImage.scale;
    imgRect.size.height *= resultingImage.scale;
    CGImageRef subImageRef = CGImageCreateWithImageInRect(resultingImage.CGImage, imgRect);
...
- (void)addConstraintsToSuperviewByEdgeInsets:(UIEdgeInsets)edgeInsets {
    ...
    if (edgeInsets.top >= 0) {
        [view addConstraint:[NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:view attribute:NSLayoutAttributeTop multiplier:1 constant:edgeInsets.top]];
    }
    if (edgeInsets.bottom >= 0) {
        [view addConstraint:[NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:view attribute:NSLayoutAttributeBottom multiplier:1 constant:-edgeInsets.bottom]];
    }
    if (edgeInsets.left >= 0) {
        [view addConstraint:[NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:view attribute:NSLayoutAttributeLeft multiplier:1 constant:edgeInsets.left]];
    }
    if (edgeInsets.right >= 0) {
        [view addConstraint:[NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:view attribute:NSLayoutAttributeRight multiplier:1 constant:-edgeInsets.right]];
    }
}
...
- (void)addConstraintsForSize:(CGSize)size {
    self.translatesAutoresizingMaskIntoConstraints = NO;
    if (size.width >= 0) {
        [self addConstraint:[NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeWidth multiplier:1 constant:size.width]];
    }
    if (size.height >= 0) {
        [self addConstraint:[NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeHeight multiplier:1 constant:size.height]];
    }
}
